...
class TestDivisionGraphRecording(unittest.TestCase):

    def test_div_records_parents(self):
        a = Tensor([8, 20])
        b = Tensor([2, 5])

        self.assertEqual((a / b).parents, (a, b))

    def test_div_records_operation(self):
        a = Tensor([8])
        b = Tensor([2])

        self.assertEqual((a / b)._op, "/")

    def test_div_propagates_requires_grad_left(self):
        a = Tensor([8], requires_grad=True)
        b = Tensor([2])

        self.assertTrue((a / b).requires_grad)

    def test_div_propagates_requires_grad_right(self):
        a = Tensor([8])
        b = Tensor([2], requires_grad=True)

        self.assertTrue((a / b).requires_grad)

    def test_div_requires_grad_false_when_both_false(self):
        a = Tensor([8])
        b = Tensor([2])

        self.assertFalse((a / b).requires_grad)

    def test_div_returns_correct_values(self):
        a = Tensor([8, 20])
        b = Tensor([2, 5])

        self.assertEqual((a / b).tolist(), [4.0, 4.0])

    def test_div_scalar_records_parent(self):
        a = Tensor([8, 20], requires_grad=True)

        self.assertEqual((a / 2).parents, (a,))

    def test_div_scalar_records_operation(self):
        a = Tensor([8])

        self.assertEqual((a / 2)._op, "/")

    def test_div_scalar_requires_grad(self):
        a = Tensor([8], requires_grad=True)

        self.assertTrue((a / 2).requires_grad)

    def test_div_scalar_returns_correct_values(self):
        a = Tensor([8, 20])

        self.assertEqual((a / 2).tolist(), [4.0, 10.0])

    def test_div_keeps_grad_none(self):
        a = Tensor([8], requires_grad=True)
        b = Tensor([2])

        self.assertIsNone((a / b).grad)

    def test_div_keeps_backward_callable(self):
        a = Tensor([8])
        b = Tensor([2])

        self.assertTrue(callable((a / b)._backward))

    def test_div_does_not_modify_inputs(self):
        a = Tensor([8], requires_grad=True)
        b = Tensor([2])
